...
export function KindIcon({ kind }: { kind: BuildEventKind }) {
  ...
  if (kind === 'active') {
    return <Loader2Icon className="animate-spin text-primary" aria-hidden />
  }
...
export function StatTile({
  ...
  const styles = {
    ...
    warn:
      'bg-amber-500/10 text-amber-800 border-amber-500/20 dark:text-amber-200',
...
export function StatusSummary({
  ...
  const { headerText, processedCount, totalCount, completedCount, skippedCount } =
    metrics
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium leading-tight">{headerText}</p>

...
export function getRecipeStatus(
  ...
  if (
    isGenerating &&
    recipeIndexById.get(recipe.widgetId) === activeIndex
  ) {
    return 'active'
  }
  return 'queued'
...
function PhaseRailHoverPlan({
  ...
  return (
    <>
      ...
      <ul className="max-h-32 list-inside list-disc space-y-0.5 overflow-y-auto text-muted-foreground">
        {preview.map((recipe) => (
          <li key={recipe.widgetId}>{recipeDisplayTitle(recipe)}</li>
        ))}
      </ul>
      {rest > 0 && (
        <p className="text-muted-foreground">+{rest} more</p>
      )}
      {recipes.slice(0, PHASE_HOVER_SOURCE_PEEK_RECIPES).map((recipe) => {
        ...
        return (
          <p key={recipe.widgetId} className="text-[11px] text-muted-foreground">
...
function PhaseRailHoverBuild({
  ...
  return (
    <>
      <p className="font-medium text-foreground">Build progress</p>
      <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 tabular-nums text-muted-foreground">
        <dt>Rendered</dt>
...
function PhaseRailHoverFinish({
  ...
  if (done) {
    return (
      <>
        <p className="font-medium text-foreground">Dashboard ready</p>
        <p className="text-muted-foreground">
...
function PhaseDot({
  ...
  if (status === 'active') {
    return (
      <div
        className={cn(base, 'border-primary bg-background text-primary', className)}
        aria-hidden
      >
...
function computePhaseSteps(
  ...
  const planStatus: PhaseStatus = recipes.length > 0
    ? 'complete'
    : isGenerating
      ? 'active'
      : 'pending'
...
export function GenerationProgressStepper({
  ...
  return (
    <div className="relative w-full min-w-0 px-2.5">
              ...
              <HoverCardTrigger asChild>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div className="aspect-square h-full max-h-36 min-h-10 min-w-10 w-auto max-w-full">
                        <PhaseDot status={step.status} />
                      </div>
                    </ItemMedia>
                    <ItemContent className="min-w-0 justify-center">
                      <ItemTitle>{step.label}</ItemTitle>
                      <span className="line-clamp-2 text-left text-sm leading-normal font-normal text-muted-foreground text-pretty">
                        {step.hint}
                      </span>
                    </ItemContent>
                  </button>
      ...
      {error && (
        <p className="mt-4 text-xs text-pretty text-destructive">{error}</p>
      )}
    </div>
  )
}

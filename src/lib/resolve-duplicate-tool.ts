...
const duplicateFieldSchema = z.object({
  ...
  proposedValue: z
    ...
    .optional()
    .describe(
      "The value that was being submitted when the conflict was detected",
    ),
  editable: z
    .boolean()
    .nullable()
    .optional()
    .describe(
      "Whether the user can edit this field to resolve the conflict (default: true)",
    ),
  conflicting: z
    .boolean()
    .nullable()
    .optional()
    .describe(
      "Whether this field is the source of the conflict — highlight it in the UI",
    ),
...
const resolutionActionSchema = z.object({
  id: z
    .string()
    .describe("Identifier returned to the agent when this action is chosen"),
  ...
  requiresEdits: z
    ...
    .optional()
    .describe(
      "When true, the edited field values are validated and included in the result",
    ),
  description: z
    .string()
    .nullable()
...
export const resolveDuplicateEntityTool = toolDefinition({
  ...
  inputSchema: z.object({
    ...
    conflictReason: z
      ...
      .optional()
      .describe(
        'Short technical reason for the conflict, e.g. "asset_name + issue_date must be unique per employee"',
      ),
    fields: z
      .array(duplicateFieldSchema)
      .describe(
        "Fields to display — show current and proposed values side by side",
      ),
    actions: z

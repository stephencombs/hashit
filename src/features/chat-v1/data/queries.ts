...
import { z } from "zod";
import { selectThreadSchema } from "../contracts/schemas";

export const threadListQuery = queryOptions({
  queryKey: ["threads"],
  queryFn: async () => {
    const { listThreads } = await import("~/features/chat-v1/server/threads");
...
export const threadDetailQuery = (threadId: string) =>
  queryOptions({
    ...
    queryFn: async ({ signal }) => {
      const { getThread } =
        await import("~/features/chat-v1/server/chat-thread-server");
...
export const artifactsByThreadQuery = (threadId: string) =>
  queryOptions({
    ...
    queryFn: async ({ signal }): Promise<ThreadArtifact[]> => {
      const { getArtifactsByThread } = await import(
        "~/features/artifacts/server/artifacts"
      );

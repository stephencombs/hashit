...
import type { AgentRunState } from "~/features/chat-v1/server/agent-runner";
import type { AppMessagePart } from "~/shared/types/message-parts";
import {
  buildArgsPreview,
  buildResultSummary,
} from "~/shared/lib/server/message-part-previews";
import { summarizeToolActivity } from "~/shared/lib/agent-runtime-utils";
...
function normalizeIncomingParts(
  ...
  return parts.map((part) => {
    ...
    if (p.type === "tool-result") {
      const tr = p as import("~/shared/types/message-parts").ToolResultPart;
      if (tr.summary !== undefined) return p;
      return { ...tr, summary: buildResultSummary(tr.content) };
    }
    if (p.type === "tool-call") {
      const tc = p as import("~/shared/types/message-parts").AppToolCallPart;

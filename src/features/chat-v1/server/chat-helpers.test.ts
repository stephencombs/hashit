...
vi.mock("~/shared/lib/agent-runtime-utils", () => ({
...
import { persistAssistantMessage } from "~/features/chat-v1/server/chat-helpers";
...
describe("persistAssistantMessage resume checkpoint persistence", () => {
  ...
  it("writes resumeOffset on thread update when checkpoint is provided", async () => {
    await (
      persistAssistantMessage as unknown as (
        ...args: Array<unknown>
      ) => Promise<string>
    )(
      "thread-1",
...
describe("persistAssistantMessage — message part shapes", () => {
  ...
  it("persists ui-spec parts with a spec object, not a stringified content field", async () => {
    ...
    const uiSpecPart = insertedValues?.parts?.find(
      (p) => p.type === "ui-spec",
    ) as { type: "ui-spec"; spec: unknown; content?: unknown } | undefined;
  ...
  it("persists tool-call parts with an argsPreview field pre-computed from arguments", async () => {
    ...
    const toolCallPart = insertedValues?.parts?.find(
      (p) => p.type === "tool-call",
    ) as { type: "tool-call"; argsPreview?: string } | undefined;

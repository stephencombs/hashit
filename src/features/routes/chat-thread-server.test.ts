...
vi.mock('@tanstack/react-start', () => ({
  createServerFn: () => ({
    inputValidator: () => ({
      handler: (handler: (input: { data: string }) => Promise<unknown>) => handler,
    }),
  }),
}))
...
import { getThread } from '~/features/routes/chat-thread-server'
...
describe('getThread durable offset hydration', () => {
  ...
  it('reads initialResumeOffset from Postgres thread row and skips durable snapshot materialization', async () => {
    ...
    expect(mocks.mockMaterializeSnapshotFromDurableStream).not.toHaveBeenCalled()
...
describe('getThread — message part shape round-trip', () => {
  ...
  it('returns ui-spec parts with a spec object (new shape) unchanged', async () => {
    ...
    expect(result.messages).toHaveLength(1)
    const uiPart = result.messages[0].parts.find((p) => p.type === 'ui-spec') as
      | { type: 'ui-spec'; spec: unknown }
      | undefined

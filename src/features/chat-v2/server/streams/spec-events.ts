...
import { V2_DURABLE_CUSTOM_EVENT_NAMES } from "../domain";
...
function isSpecCompleteChunk(chunk: DurableChunk): chunk is DurableChunk & {
  type: "CUSTOM";
  name: typeof V2_DURABLE_CUSTOM_EVENT_NAMES.specComplete;
  value: { spec: Spec; specIndex: number };
} {
  if (
    chunk.type !== "CUSTOM" ||
    chunk.name !== V2_DURABLE_CUSTOM_EVENT_NAMES.specComplete
  ) {
    return false;
  }
...
export async function readV2UiSpecEventsByMessageId(
  threadId: string,
): Promise<Map<string, Array<V2UiSpecEventPart>>> {
  ...
  const byMessageId = new Map<string, Array<V2UiSpecEventPart>>();

...
import type { FormSpec, FormField } from "~/shared/lib/form-tool";
...
function FieldLabel({
  ...
  return (
    <Label
      htmlFor={id}
      className="text-foreground/90 text-[12.5px] leading-none font-medium tracking-tight"
    >
      {label}
      {required && (
            ...
            if (field.type === "checkbox") {
              return (
                ...
                >
                  <Label
                    htmlFor={field.name}
                    className="text-foreground cursor-pointer text-[13px] leading-snug"
                  >
                    {field.label}
                    {field.required && (
                      <span className="text-destructive/90 ml-0.5" aria-hidden>

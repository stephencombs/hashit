...
export default defineTask({
  ...
  async run() {
    ...
    if (latest?.status === 'generating') {
      console.log(`[dashboard:check] Snapshot ${latest.id} is still generating, skipping`)
    ...
    if (!isStale) {
      console.log(`[dashboard:check] Latest snapshot ${latest!.id} is fresh (${Math.round((Date.now() - latest!.createdAt.getTime()) / 60_000)}m old), skipping`)
    ...
    console.log(`[dashboard:check] Triggering generation for snapshot ${snapshotId} (previousWidgetIds: ${previousWidgetIds.length})`)

    await generateDashboard({ snapshotId, persona, previousWidgetIds, previousWidgets })
